...
function MsrView({ theoryId, apiBase }: { theoryId: number; apiBase: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Indicator Scoring — collapsible section */}
      <div className="rounded-xl border border-slate-200 overflow-hidden">
        <button
          onClick={() => setMatrixOpen(o => !o)}
          className="w-full flex items-center gap-2 px-4 py-2.5 bg-slate-50 hover:bg-slate-100 transition-colors text-left border-b border-slate-100">
          <ListChecks className="w-3.5 h-3.5 text-slate-400 shrink-0" />
          <span className="text-xs font-semibold text-slate-600 flex-1">Indicator Scoring</span>
          {matrixOpen ? <ChevronUp className="w-3.5 h-3.5 text-slate-400" /> : <ChevronDown className="w-3.5 h-3.5 text-slate-400" />}
        </button>

      <div className={matrixOpen ? "p-4 space-y-4" : "hidden"}>
